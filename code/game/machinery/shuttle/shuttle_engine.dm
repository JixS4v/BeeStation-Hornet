...
/obj/machinery/shuttle/engine/proc/fireEngine()
	...
	var/heat_cap = env.getHeatCapacity()
	var/req_power = abs(env.get_temperature() - ENGINE_HEAT_TARGET) * heat_cap
	req_power = min(req_power, ENGINE_HEATING_POWER)
	var/deltaTemperature = req_power / heat_cap
	if(deltaTemperature < 0)
		return
	env.set_temperature(env.get_temperature() + deltaTemperature)
	air_update_turf()
