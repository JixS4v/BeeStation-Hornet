...
/obj/machinery/atmospherics/components/unary/vent_pump/process_atmos()
	...
	if(pump_direction & RELEASING) // internal -> external
		...
		if(pressure_delta > 0)
			if(air_contents.get_temperature() > 0 && air_contents.return_volume() > 0)
				var/transfer_moles = pressure_delta*environment.return_volume()/(air_contents.get_temperature() * R_IDEAL_GAS_EQUATION)
		...
		if(environment.returnPressure() > 0)
			var/our_multiplier = air_contents.return_volume() / (environment.get_temperature() * R_IDEAL_GAS_EQUATION)
			var/moles_delta = 10000 * our_multiplier
			if(pressure_checks&EXT_BOUND)
				moles_delta = min(moles_delta, (environment_pressure - external_pressure_bound) * environment.return_volume() / (environment.get_temperature() * R_IDEAL_GAS_EQUATION))
			if(pressure_checks&INT_BOUND)
				moles_delta = min(moles_delta, (internal_pressure_bound - air_contents.returnPressure()) * our_multiplier)
