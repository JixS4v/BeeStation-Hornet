...
/datum/reagent/consumable/frostoil/reaction_turf(turf/T, reac_volume)
	...
	if(reac_volume >= 1) // Make Freezy Foam and anti-fire grenades!
		if(isopenturf(T))
			...
			OT.air.set_temperature(OT.air.get_temperature() - MOLES_CELLSTANDARD*100*reac_volume/OT.air.getHeatCapacity()) // reduces environment temperature by 5K per unit.
...
/datum/reagent/consumable/cornoil/reaction_turf(turf/open/T, reac_volume)
	...
	if(hotspot)
		...
		lowertemp.set_temperature(max( min(lowertemp.get_temperature()-2000,lowertemp.get_temperature() / 2) ,TCMB))
		lowertemp.react(src)
		qdel(hotspot)
